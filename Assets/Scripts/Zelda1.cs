...
public class Zelda1 : MonoBehaviour
{
    ...
    void Update()
    {
        grounded = (Physics2D.Linecast(transform.position, groundcheck.position, LayerMask.NameToLayer("stage")) || Physics2D.Linecast(transform.position, groundcheck.position, LayerMask.NameToLayer("backstage")));
        Debug.Log(grounded);
        if (grounded == true)
        {
